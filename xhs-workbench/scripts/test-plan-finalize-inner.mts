...
import { generateTopicOptions, getCapabilityFallback } from '../src/lib/v2/topic-stage';
...
const output = path.resolve('data/plan-finalize-inner/2026-09-06');
...
const topicKeys = ['finalPublicTopic', 'promise'].sort();
...
const planFn = contentTree.statements.find(n => ts.isFunctionDeclaration(n) && n.name?.text === 'generateStandardKnowledgePagePlan') as ts.FunctionDeclaration;
assert.ok(planFn.modifiers?.some(m => m.kind === ts.SyntaxKind.ExportKeyword));
const planDeclarations = planFn.body!.statements.filter(ts.isVariableStatement).flatMap(n => [...n.declarationList.declarations]);
const planInput = planDeclarations.find(d => d.name.getText() === 'planInput')!.initializer!;
assert.deepEqual(objectKeys(planInput), topicKeys);
const functionText = (name: string) => {
  const node = contentTree.statements.find(n => ts.isFunctionDeclaration(n) && n.name?.text === name)!;
  return node.getText(contentTree);
};
const localCode = ts.transpileModule(functionText('normalizePagePlan') + '\n' + functionText('clean'), {
  compilerOptions: { target: ts.ScriptTarget.ES2020, module: ts.ModuleKind.None },
}).outputText;
const normalizeFromProduction = new Function(localCode + '; return normalizePagePlan;')() as (v: unknown) => any[];
const normalizedValue = (value: unknown) => normalizeFromProduction([{ pageId: 'p1', pageGoal: '目标', userGets: '资产', pageContentPlan: value }]);
assert.equal(normalizedValue('  保留  内部空白\n以及换行  ')[0].pageContentPlan, '保留  内部空白\n以及换行');
assert.equal(normalizedValue([' 给3组表达 ', '', '  ', '每组完整例句', ' 附中文说明 '])[0].pageContentPlan, '给3组表达；每组完整例句；附中文说明');
for (const value of [null, undefined, 1, {}, [1], ['文字', {}], [{ text: '文字' }], [], [' ', '']]) assert.equal(normalizedValue(value).length, 0);
const proof = { formatCompatibilityUnitChecks: 'PASS', planInputKeys: topicKeys, planInvokedInsideExistingContentFunction: true, innerNetworkCallsAllowed: 3, checkedBeforeNetwork: true, topKeys, topicKeys, pageKeys, evidenceKeys,
...
globalThis.fetch = async (url, init) => {
  ...
  assert.ok(stage && currentCase, 'Only Topic/Page Plan/Inner allowed; downstream blocked before network.');
  const key = `${currentCase}/${stage}`;
  const attempt = (attempts[key] || 0) + 1;
  assert.equal(attempt, 1, 'One actual request per node in this experiment; no rerun.');
  assert.ok(observed.length < 9, 'Nine-request maximum.');
  assert.equal(body.model, 'qwen3.7-flash');
  assert.equal(body.enable_thinking, false);
  assert.deepEqual(body.response_format, { type: 'json_object' });
  const user = JSON.parse(body.messages[1].content);
  if (stage === 'page_plan') {
    assert.deepEqual(Object.keys(user).sort(), topicKeys);
    const generated = JSON.parse(await fs.readFile(path.join(output, currentCase, 'topic-artifact.json'), 'utf8')).data[0];
    assert.deepEqual(user, { finalPublicTopic: generated.topic, promise: generated.promise });
  }
  if (stage === 'inner') {
    assert.deepEqual(Object.keys(user).sort(), topKeys);
    assert.deepEqual(Object.keys(user.topic).sort(), topicKeys);
    const rawPlan = JSON.parse(await fs.readFile(path.join(output, currentCase, 'page_plan-1', 'parsed.json'), 'utf8'));
    const normalized = normalizeFromProduction(rawPlan.pages);
    assert.deepEqual(user.canonical_page_plan, normalized);
    assert.ok(user.canonical_page_plan.length);
    user.canonical_page_plan.forEach((p: any) => {
      assert.deepEqual(Object.keys(p).sort(), pageKeys);
      assert.equal(typeof p.pageContentPlan, 'string');
    });
    assert.equal(body.messages[0].content, oldInner);
    assert.equal(body.temperature, 0.72);
    assert.equal(body.max_tokens, 10000);
    await save(`${currentCase}/normalized-plan.json`, user.canonical_page_plan);
    await save(`${currentCase}/format-verification.json`, {
      rawTypes: rawPlan.pages.map((p: any) => Array.isArray(p.pageContentPlan) ? 'array' : typeof p.pageContentPlan),
      finalTypes: user.canonical_page_plan.map((p: any) => typeof p.pageContentPlan),
      equalsProductionNormalization: true,
    });
  }
...
try {
  for (const item of cases) {
    ...
    try {
      ...
      results.push({ case: item.id, status: 'FAILED', errorName: e.name, failure: e.message === 'Topic AI未返回有效选题' ? 'TOPIC_OUTPUT_CONTRACT_FAILURE' : 'NODE_OR_TRANSPORT_FAILURE', usage: e.usage });
    }
    console.log(`CASE_DONE ${item.id} ${results.at(-1)?.status}`);
  }
} finally {
  globalThis.fetch = nativeFetch;
  await save('summary.json', { observed, results, httpCalls: observed.length, innerCalls: observed.filter(r => r.stage === 'inner').length });
}

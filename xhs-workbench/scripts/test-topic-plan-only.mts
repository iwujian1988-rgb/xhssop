...
import { generateStandardKnowledgePagePlan } from '../src/lib/v2/content-stage';

nextEnv.loadEnvConfig(process.cwd());
const output = path.resolve('data/topic-plan-only/2026-09-06-final-cleanup');
...
const proof = { planInputKeys: topicKeys, directExportedProductionPlan: true, innerNetworkCallsAllowed: 0, checkedBeforeNetwork: true, topKeys, topicKeys, pageKeys, evidenceKeys,
...
globalThis.fetch = async (url, init) => {
  ...
  assert.ok((stage === 'topic' || stage === 'page_plan') && currentCase, 'Only Topic/Page Plan allowed; Inner and downstream blocked in test before network.');
  ...
  assert.ok(observed.length < 6, 'Six-request maximum.');
...
try {
  for (const item of cases) {
    ...
    try {
      ...
      await save(`${item.id}/plan-artifact.json`, plan);
      results.push({ case: item.id, status: 'GENERATED', topicUsage: generatedTopic.usage, planUsage: plan.usage });
    } catch (error) {
  ...
  await save('summary.json', { observed, results, httpCalls: observed.length, innerCalls: 0 });
}

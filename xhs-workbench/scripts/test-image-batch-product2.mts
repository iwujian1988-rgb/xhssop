...
/**
 * 商品2 image_to_image 验证脚本：跑含 08/12/13 三个 image_to_image 模板的 batch，
 * 验证服务端出图链路（compose → generateCoverImageWithRetry → 落盘 cover_image_url）。
 *
 * 用法：npx tsx scripts/test-image-batch-product2.mts
...
async function main() {
  console.log(`[image-test] base=${BASE_URL} cards=${IMAGE_CARDS.join(',')} product=tef_tcf_canada`);
  ...
  const outPath = `image-batch-product2-result-${Date.now()}.json`;
...
main().catch(error => {
  console.error('image-test (product2) crashed:', error);
  process.exit(1);
});

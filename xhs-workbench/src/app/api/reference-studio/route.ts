...
import { callOpenAICompatibleJson, getRecentAiUsage, resetRecentAiUsage } from '@/lib/ai-client';
...
async function generateTopics(input: {
  ...
  const topics = Array.isArray(root.topics) ? root.topics.map(normalizeTopic).filter(Boolean) : [];
  if (topics.length < 3) throw new Error('AI没有返回3个可用迁移选题');
  return topics.slice(0, 3);
}

async function composeDraft(input: {
  productId: ProductId;
  card: NonNullable<ReturnType<typeof getCompetitorCreativeCard>>;
  topic: MigratedTopic;
...
async function auditEducationalContent(input: {
  productId: ProductId;
  cover: ReturnType<typeof normalizeDenseDirectoryCover>;
  innerPages: GeneratedInnerPage[];
  evidence: ReturnType<typeof retrieveProductFacts>;
  renderer: NonNullable<ReturnType<typeof getCompetitorCreativeCard>>['renderer_id'];
}) {
  ...
  const result = asRecord(await callOpenAICompatibleJson([
    {
      ...
      content: [
        ...
        '每条issue的problem必须给出单一、确定的结论，禁止先说"有错误"又说"无错误/无误/正确/无问题/应改为warning/不是错误/不构成错误"这种自我否定或自我改判的表述；如果分析到最后发现其实没有错误、只是更地道的表达偏好、或者是封面短条目允许的词干/未完整礼貌结语，直接把severity设为warning或完全不放进issues数组，不要标severity为error后又在problem里说它其实不构成错误。',
...
function generateEditorialOutput(input: Parameters<typeof composeDraft>[0]) {
...
function normalizeBrief(value: unknown, input: Parameters<typeof composeDraft>[0]): UnifiedContentBrief {
...
function getCoreIssues(titles: TitleCandidate[], cover: ReturnType<typeof normalizeDenseDirectoryCover>, renderer: NonNullable<ReturnType<typeof getCompetitorCreativeCard>>['renderer_id']) {
  const issues: string[] = [];
  const spec = getCoverTemplateSpec(renderer);
  const itemCount = cover.sections.reduce((sum, section) => sum + section.items.length, 0);
...
function normalizeTags(value: unknown, seoKeywords: string[]) {
  ...
  const normalized = [...raw, ...fallbacks]
    ...
    .filter(tag => !/^#(AU|CH|FW|GD|B\d)[-_]?\w*/i.test(tag) && tag.length >= 3 && tag.length <= 18);

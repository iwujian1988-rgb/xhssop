...
export const AI_CLICHE_PATTERN = /不是.{0,40}而是|不在于.{0,40}而在于|问题(?:就)?出在|问题的关键|不仅仅是.{1,18}.{0,4}更是|在.{1,18}的过程中|才是.{1,12}(?:关键|核心|根本)|通过.{1,18}，.{1,12}才能|让.{1,12}不再|别让.{0,12}拖后腿|(?:又|总是)?卡(?:了|主了|住了)|守住(?:这|那)?一分|替换就守住|重要性不言而喻|是一个需要.{1,18}的过程|综上所述|^总而言之|^总的来说|首先[，,][^。]{0,80}其次[，,][^。]{0,80}最后[，,]/;

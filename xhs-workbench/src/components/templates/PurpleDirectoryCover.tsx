...
function CleanPurpleDirectory({ payload, className }: { payload: DenseDirectoryCoverPayload; className: string }) {
  // 紫色参考页靠分组和多列保持密度，不能靠无限缩小字号来容纳内容。
  const fitRef = useAutoFitScale<HTMLDivElement>([sectionsFingerprint(payload)], { min: 0.68, max: 1, step: 0.025 });
  return <article className={`clean-purple-directory ${className}`} style={{ '--section-count': Math.max(payload.sections.length, 1) } as React.CSSProperties} aria-label={payload.title}>
    ...
    <div className="clean-purple-sections" ref={fitRef} style={{ justifyContent: 'space-between', gap: '1.2cqw' }}>{payload.sections.map((section, sectionIndex) => <section key={`${section.heading}-${sectionIndex}`}>
    ...
    <style>{`
      .clean-purple-directory{position:relative;width:100%;aspect-ratio:3/4;overflow:hidden;border:1px solid #e5dfeb;background:#fff;box-shadow:0 22px 50px rgba(45,25,55,.18);color:#673287;container-type:inline-size;font-family:"PingFang SC","Microsoft YaHei","Noto Sans SC",sans-serif;letter-spacing:0}
      .clean-purple-directory *{box-sizing:border-box;min-width:0}.clean-purple-directory header{min-height:13%;padding:3.2cqw 5.2cqw 1.2cqw;text-align:center}.clean-purple-directory h1{margin:0;color:#68308a;font-family:"Source Han Serif SC Heavy","Noto Serif SC",serif;font-size:6.6cqw;font-weight:900;line-height:1.08;white-space:normal;overflow-wrap:anywhere}.clean-purple-directory header p{margin:.75cqw 0 0;color:#9d426c;font-family:"Noto Sans SC","Microsoft YaHei",sans-serif;font-size:2.05cqw;font-weight:800}
      .clean-purple-sections{display:flex;flex-direction:column;justify-content:flex-start;gap:2.1cqw;height:85%;padding:0 5.6cqw 3.6cqw;overflow:hidden}.clean-purple-sections section{display:flex;flex-shrink:0;flex-direction:column;justify-content:flex-start}.clean-purple-sections h2{margin:0 0 calc(.85cqw * var(--fit-scale, 1));color:#71318d;font-family:"Source Han Serif SC Heavy","Noto Serif SC",serif;font-size:clamp(13px, calc(3.3cqw * var(--fit-scale, 1)), 26px);font-weight:900;line-height:1.05}.clean-purple-bracket{position:relative;margin-left:6.1cqw;padding:calc(1.15cqw * var(--fit-scale, 1)) 0 calc(1.15cqw * var(--fit-scale, 1)) 3cqw}.clean-purple-bracket:before{content:"";position:absolute;top:.4cqw;bottom:.4cqw;left:0;width:1.35cqw;border-top:.32cqw solid #73318f;border-bottom:.32cqw solid #73318f;border-left:.32cqw solid #73318f}
      .clean-purple-grid{display:grid;grid-template-columns:repeat(var(--columns),minmax(0,1fr));column-gap:2cqw;row-gap:calc(.82cqw * var(--fit-scale, 1))}.clean-purple-item{display:grid;grid-template-columns:1.3cqw minmax(0,1fr);align-items:start;font-size:clamp(11px, calc(3cqw * var(--fit-scale, 1)), 24px);font-weight:700;line-height:1.12}.clean-purple-item i{width:.78cqw;height:.78cqw;margin-top:.48cqw;border-radius:50%;background:#77318f}.clean-purple-item span{display:flex;flex-direction:column;gap:.1cqw;min-width:0}.clean-purple-item strong{display:block;color:#9f426c;font-family:Georgia,"Noto Sans SC",sans-serif;font-weight:700;overflow-wrap:anywhere}.clean-purple-item em{display:block;margin-left:0;color:#673287;font-family:"PingFang SC","Microsoft YaHei","Noto Sans SC",sans-serif;font-size:.88em;font-style:normal;font-weight:800;overflow-wrap:anywhere}.clean-purple-page{position:absolute;top:.7cqw;right:.7cqw;display:grid;width:6.3cqw;height:6.3cqw;place-items:center;border-radius:50%;background:rgba(105,105,105,.72);color:#fff;font:800 2.1cqw Arial,sans-serif}
    `}</style>
  </article>;
}
...
function GridPurpleDirectory({ payload, className }: { payload: DenseDirectoryCoverPayload; className: string }) {
  ...
  const fitRef = useAutoFitScale<HTMLDivElement>([sectionsFingerprint({ ...payload, sections })], { min: 0.68, max: 1, step: 0.025 });
  return <article className={`grid-purple-sheet ${className}`} aria-label={payload.title}>
    ...
    <style>{`
      .grid-purple-sheet{position:relative;box-sizing:border-box;width:100%;aspect-ratio:3/4;overflow:hidden;border:1px solid #d9d2e0;background-color:#fbfaf0;background-image:linear-gradient(rgba(117,80,145,.12) 1px,transparent 1px),linear-gradient(90deg,rgba(117,80,145,.12) 1px,transparent 1px);background-size:3.8cqw 3.8cqw;box-shadow:0 22px 50px rgba(50,30,70,.2);color:#3e2d49;container-type:inline-size;font-family:"PingFang SC","Microsoft YaHei","Noto Sans SC",sans-serif;letter-spacing:0}
      .grid-purple-sheet *{box-sizing:border-box}.grid-purple-sheet header{min-height:14%;padding:3.2cqw 3.5cqw 1.2cqw 8.5cqw;text-align:center}.grid-purple-sheet h1{margin:0;color:#5d2a86;font-family:"Source Han Serif SC Heavy","Noto Serif SC",serif;font-size:7.5cqw;font-weight:900;line-height:1.08;white-space:normal;overflow-wrap:anywhere}.grid-purple-sheet header p{margin:.8cqw 0 0;color:#a34b76;font-family:"Noto Sans SC","Microsoft YaHei",sans-serif;font-size:2.45cqw;font-weight:800}.grid-purple-sections{display:flex;flex-direction:column;justify-content:flex-start;gap:2.1cqw;height:82%;padding:0 3.5cqw 2.5cqw 8.5cqw;overflow:hidden}.grid-purple-sections section{display:flex;flex-shrink:0;flex-direction:column;justify-content:flex-start}.grid-purple-sections h2{margin:0 0 calc(.65cqw * var(--fit-scale, 1));color:#713184;font-family:"Source Han Serif SC Heavy","Noto Serif SC",serif;text-align:left;font-size:clamp(13px, calc(3.1cqw * var(--fit-scale, 1)), 25px);line-height:1.05}.grid-purple-table{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border-top:.18cqw solid #70458e;border-left:.18cqw solid #70458e;background:rgba(255,255,255,.5)}.grid-purple-sections section:nth-child(n+3) .grid-purple-table{grid-template-columns:repeat(2,minmax(0,1fr))}.grid-purple-table>div{display:flex;min-height:calc(4.2cqw * var(--fit-scale, 1));align-items:center;gap:.4cqw;padding:.35cqw .7cqw;border-right:.18cqw solid #70458e;border-bottom:.18cqw solid #70458e;flex-wrap:wrap;font-size:clamp(10px, calc(2.15cqw * var(--fit-scale, 1)), 17px);line-height:1.12}.grid-purple-table strong{display:block;color:#3e2949;overflow-wrap:anywhere}.grid-purple-table span{display:block;color:#a84572;font-size:.85em;overflow-wrap:anywhere}.grid-purple-tools{position:absolute;top:0;bottom:0;left:0;display:flex;width:6.4cqw;flex-direction:column;align-items:center;gap:3.2cqw;padding-top:5cqw;border-right:.12cqw solid rgba(105,65,139,.16);background:rgba(255,255,255,.52);font-family:Arial,sans-serif}.grid-purple-tools b{font-size:2.4cqw;font-weight:500}.grid-purple-tools i{width:2.25cqw;height:2.25cqw;border-radius:50%;background:#62c5d8}.grid-purple-tools i:nth-of-type(2){background:#1fa9dc}.grid-purple-tools i:nth-of-type(3){background:#ffd94c}.grid-purple-tools i:nth-of-type(4){background:#e8ad62}.grid-purple-tools i:nth-of-type(5){background:#222}
      .grid-purple-sections{height:81.2%}
    `}</style>
  </article>;
}

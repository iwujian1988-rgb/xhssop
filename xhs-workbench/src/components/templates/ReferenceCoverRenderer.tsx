...
function PlainExperience({ payload, className }: CoverProps) {
  ...
  return <Frame className={`rc-experience ${className}`} label={payload.title} style={{ '--exp-title': titleSize } as CSSProperties}><div className="rc-rule"/><h1>{payload.title}</h1><p className="rc-experience-lead">{payload.subtitle}</p><div className="rc-experience-body" ref={fitRef}>{paragraphs.map((paragraph,index)=><p className="rc-experience-paragraph" key={index}>{paragraph.replace(/[。]+$/u, '')}。</p>)}</div></Frame>;
}
